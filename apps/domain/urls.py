...
urlpatterns = patterns( 
        'domain.views',        
        url(r'^select/$', 'select', name='domain_select'),
        ...
        url(r'^registration_request(?:/(?P<kind>\w+))?/$', 'registration_request', name='domain_registration_request'),                                   
        
        # Same trick as above - make GUID optional
        url(r'^registration_confirm(?:/(?P<guid>\w+))?/$', 'registration_confirm',  name='domain_registration_confirm'),        
        url(r'^registration_resend_confirm_email/$', 'registration_resend_confirm_email', name='domain_registration_resend_confirm_email'),
        
        # domain admin functions
        url(r'^admin/$', 'admin_main', name='domain_admin_main'),
        url(r'^admin/user_list/$', 'user_list', name='domain_user_list'),
        url(r'^admin/edit_user/(?P<user_id>\d{1,10})/$', 'edit_user', name='domain_edit_user'),        
        )
